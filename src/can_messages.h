...
namespace ChargingMessages {
    ...
    const CANSignal_t SIG_PLUG_CONNECTED = {
        "PlugConnected", 0, 1, 1, 0, "bool", "charging/plug_connected", 10000
    };
    const CANSignal_t SIG_CHARGE_POWER = {
        "ChargePower", 8, 16, 0.1, 0, "kW", "charging/power", 60000
    };
    const CANSignal_t SIG_CHARGE_VOLTAGE = {
        "ChargeVoltage", 24, 16, 0.1, 0, "V", "charging/voltage", 60000
    };
    const CANSignal_t SIG_CHARGE_CURRENT = {
        "ChargeCurrent", 40, 16, 0.1, 0, "A", "charging/current", 60000
    };
}
...
namespace MotionMessages {
    ...
    const CANSignal_t SIG_VEHICLE_SPEED = {
        "Speed", 0, 16, 0.01, 0, "km/h", "motion/speed", 10000
    };
    const CANSignal_t SIG_BRAKE_PRESSURE = {
        "BrakePressure", 16, 16, 0.01, 0, "bar", "motion/brake_pressure", 10000
    };

    // 0x0154 - Motor RPM & Power
    const uint32_t MSG_MOTOR_STATUS = 0x0154;
    const CANSignal_t SIG_MOTOR_RPM = {
        "MotorRPM", 0, 16, 1, 0, "rpm", "motion/motor_rpm", 10000
    };
    const CANSignal_t SIG_MOTOR_TORQUE = {
        "MotorTorque", 16, 16, 0.1, -3276.8, "Nm", "motion/motor_torque", 10000
    };

    // 0x119 - Reconstruction (consumption)
    const uint32_t MSG_CONSUMPTION = 0x119;
    const CANSignal_t SIG_CONSUMPTION_KWH = {
        "ConsumptionKWh", 0, 16, 0.01, 0, "kWh/100km", "motion/consumption_kwh_100km", 60000
    };
    const CANSignal_t SIG_CONSUMPTION_INSTANT = {
        "InstantConsumption", 16, 16, 0.01, 0, "kW", "motion/consumption_instant", 10000
    };

    // 0x100 - Distances & Ranges
    const uint32_t MSG_RANGE = 0x100;
    const CANSignal_t SIG_AVAILABLE_RANGE = {
        "AvailableRange", 0, 16, 1, 0, "km", "motion/available_range", 60000
    };
    const CANSignal_t SIG_TRIP_DISTANCE = {
        "TripDistance", 16, 32, 0.01, 0, "km", "motion/trip_distance", 60000
    };
}
...
namespace ClimateMessages {
    ...
    const CANSignal_t SIG_INTERIOR_TEMP = {
        "InteriorTemp", 0, 8, 0.5, -40, "°C", "climate/interior_temp", 300000
    };

    // 0x65F - Heat Pump Data
    const uint32_t MSG_HEAT_PUMP = 0x65F;
    const CANSignal_t SIG_HP_PRESSURE = {
        "HPPressure", 0, 16, 0.1, 0, "bar", "climate/heat_pump_pressure", 300000
    };
    const CANSignal_t SIG_HP_EVAP_TEMP = {
        "HPEvapTemp", 16, 8, 1, -40, "°C", "climate/heat_pump_evap_temp", 300000
    };
    const CANSignal_t SIG_HP_COND_TEMP = {
        "HPCondTemp", 24, 8, 1, -40, "°C", "climate/heat_pump_cond_temp", 300000
    };
}
...
namespace TPMSMessages {
    ...
    const CANSignal_t SIG_TIRE_FL_PRESSURE = {
        "TireFL_Pressure", 0, 8, 0.5, 0, "bar", "tpms/tire_fl_pressure", 300000
    };
    const CANSignal_t SIG_TIRE_FR_PRESSURE = {
        "TireFR_Pressure", 8, 8, 0.5, 0, "bar", "tpms/tire_fr_pressure", 300000
    };
    const CANSignal_t SIG_TIRE_RL_PRESSURE = {
        "TireRL_Pressure", 16, 8, 0.5, 0, "bar", "tpms/tire_rl_pressure", 300000
    };
    const CANSignal_t SIG_TIRE_RR_PRESSURE = {
        "TireRR_Pressure", 24, 8, 0.5, 0, "bar", "tpms/tire_rr_pressure", 300000
    };
}
...
namespace PowerMessages {
    ...
    const CANSignal_t SIG_12V_VOLTAGE = {
        "Voltage12V", 0, 16, 0.01, 0, "V", "power/voltage_12v", 300000
    };
    const CANSignal_t SIG_24V_VOLTAGE = {
        "Voltage24V", 16, 16, 0.01, 0, "V", "power/voltage_24v", 300000
    };

    // 0x35F - HV Power Module Temp
    const uint32_t MSG_POWER_MODULE_TEMP = 0x35F;
    const CANSignal_t SIG_POWER_MODULE_TEMP = {
        "PowerModuleTemp", 0, 8, 1, -40, "°C", "power/power_module_temp", 300000
    };
}
...
namespace RecuperationMessages {
    ...
    const CANSignal_t SIG_MAX_RECUP = {
        "MaxRecupPower", 0, 16, 0.1, 0, "kW", "recuperation/max_power", 60000
    };
    const CANSignal_t SIG_INSTANT_RECUP = {
        "InstantRecup", 16, 16, 0.1, 0, "kW", "recuperation/instant_power", 10000
    };
    const CANSignal_t SIG_TOTAL_RECUP = {
        "TotalRecup", 32, 32, 0.01, 0, "kWh", "recuperation/total_energy", 300000
    };
}
